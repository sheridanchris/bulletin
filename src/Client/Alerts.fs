...
let private successAlert (alertModel: AlertModel) =
  html
    $"""
    <div class="flex p-4 mb-4 text-sm text-green-700 bg-green-100 rounded-lg dark:bg-green-200 dark:text-green-800" role="alert">
      ...
      <div>
        <span class="font-medium">{alertModel.Reason}</span>
      </div>
    </div>
    """

let private infoAlert (alertModel: AlertModel) =
  html
    $"""
...
let private warningAlert (alertModel: AlertModel) =
  html
    $"""
...
let private dangerAlert (alertModel: AlertModel) =
  html
    $"""
...
let AlertComponent (alert: Alert) =
  match alert with
  | Success alert -> successAlert alert
  | Info alert -> infoAlert alert
  | Warning alert -> warningAlert alert
  | Danger alert -> dangerAlert alert
