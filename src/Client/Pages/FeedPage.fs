...
let Pagination
  ...
  let nextButton () =
    html
      $"""
      <button
        @click={Ev(fun _ -> goToNext ())}
        class="px-3 py-2 ml-0 leading-tight text-gray-500 bg-white border border-gray-300 rounded-l-lg hover:bg-gray-100
        hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Next</button>
      """
        ...
        <div class="w-full sm:w-3/12">
            ...
            <div class="relative">
                ...
                <input
                  ...
                  placeholder="Search">
                <button
                  @click={Ev(fun _ -> ApplicationContext.dispatch ApplicationContext.Search)}
                  type="submit"
                  class="text-white absolute right-2.5 bottom-2.5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300
                  font-medium rounded-lg text-sm px-4 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Search</button>
            </div>
        </div>
        <select
          @change={EvVal(ApplicationContext.SetOrdering >> ApplicationContext.dispatch)}
          id="ordering"
          class="w-full sm:w-1/12 block px-4 py-3 text-base text-gray-900 border border-gray-300 rounded-lg bg-gray-50
